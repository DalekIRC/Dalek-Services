...
hook::func("raw", function($u)
{
	...
	$nick = new User($parv[2]);
	if (!isset($nick->account))
		return;
	if ($nick->account == $nick->nick)
		$ns->sendraw(":$ns->nick SVS2MODE $nick->nick +r");
});
...
hook::func("raw", function($u)
{
	...
	if ($parv[1] == "TOPIC")
		$string = rparv(rparv($u['string']));
	
	elseif ($parv[0] == "TOPIC")
		$string = rparv($u['string']);
	...
	$topic = mb_substr(rparv(rparv(rparv($string))),1);
	
	update_topic($chan,$topic);
...
function update_topic($chan,$topic)
{
	$conn = sqlnew();
	
	$prep = $conn->prepare("UPDATE dalek_channels SET topic = ? WHERE channel = ?");
