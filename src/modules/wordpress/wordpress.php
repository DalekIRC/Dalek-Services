...
define("LKUP_BY_ID", "0-001");
define("LKUP_BY_ACCOUNT_NAME", "0-002");
define("LKUP_BY_EMAIL", "0-003");
class WPUser {
	...
	function __construct($account = "", $searchType = NULL)
	{
		...
		if ($searchType)
		{
			if ($searchType == LKUP_BY_ID)
				$nick = $this->lookup_by_id($account);

			elseif ($searchType == LKUP_BY_ACCOUNT_NAME)
				$nick = $this->lookup($account);

			elseif ($searchType == LKUP_BY_EMAIL)
