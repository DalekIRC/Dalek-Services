...
class botserv {
	...
	/* Module handle */
	public $name = "botserv"; 	/* $name needs to be the same name as the class and file lol */
	public $description = "BotServ PseudoClient"; /* $description of what the module does. This will show in the MODULE response to the server. */
	...
	function __destruct()
	{
		global $botserv; /* The global array of botserv.conf settings */
		
		/* Find our client BotServ
		 * Client::find(String $nick);
		 */
		$bs = Client::find($botserv['nick']);
		 ...
		 */
		hook::del("connect", 'botserv::spawn_client');
	}
	...
	function __init() : bool
	{
		 ...
		 * @param2 Reference to the method in this class
		 * 
		 * Spawning our client based on our botserv.conf
		 */
		hook::func("connect", 'botserv::spawn_client');
		 ...
		 */
		if (IsConnected())
			if (!botserv::spawn_client()) /* Spawning the client went wrong, return false */
				return false;
	...
	static function spawn_client()
	{
		...
		/* spawn client and store it in $bs
		 *
		 * new Client();
		 * @param1 String $nick
		 * @param2 String $ident
